...
def format_pile(stones_in_pile: int) -> str:
  ...
  """
  #stones_in_pile = ""
  max = math.ceil(math.sqrt(stones_in_pile))
  count = 0
  for i in range (1, stones_in_pile, 1):
    print("*", end=' ')
    count += 1
    if(count == max):
      print("\n")
  ...
  if stones_in_pile == 1:
    return "There is 1 stone in the pile"
...
def get_ai_guess(stones_in_pile:int, valid_guesses:list[int]) -> int:
  ...
  """if (stones_in_pile - 11) < 5 or stones_in_pile - 5 < 5:
    if(stones_in_pile - 11) < 5:
      return stones_in_pile-11
    elif (stones_in_pile - 5) < 5:
      return stones_in_pile-5"""
